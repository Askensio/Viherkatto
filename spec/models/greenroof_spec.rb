...
describe Greenroof do
  ...
  before do
    ...
    @groof = Greenroof.new(address: "Kumpulan kampus", purpose: 0, year: 2010, locality: "helsinki", usage_experience: "Oli kivaa jee", status: 1,
  ...
  describe "when status is not numerical" do
    before { @groof.status = "roolini" }
    it { should_not be_valid }
  end

  describe "when status isn't between valid values" do
    before { @groof.status = 50 }
    it { should_not be_valid }
  end

  describe "when status is negative" do
    before { @groof.status = -1 }
    it { should_not be_valid }
  end
