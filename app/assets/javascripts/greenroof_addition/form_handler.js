...
function init() {
    ...
    $('#add-layer-button-1').click(addLayer)
    $('#save').click(save)
...
var addLayer = function addLayer(event) {
    ...
    var layerCount = baseFormElement.children('div').size() + 1
    var layerElement = generateLayerForm(layerCount)
    baseFormElement.append(layerElement)
}

function generateLayerForm(layerCount) {

    // creates a new container for the layer form
    var layerFormElement = $('<div></div>').attr('id', 'layer' + layerCount)
    // adds the header
    var h2Element = $('<h2>Kerros ' + layerCount + '</h2>')
    layerFormElement.append(h2Element)
    ...
    inputAndLabelFields.children('h2').remove()
    // clears the fields
